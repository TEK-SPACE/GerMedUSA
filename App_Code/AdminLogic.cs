        ...
        static public String EntityEditPageFormHandler(AspDotNetStorefront.SkinBase sb, EntitySpecs specs, Customer ThisCustomer, int SkinID)
        {
            ...
            if (CommonLogic.FormBool("IsSubmit"))
            {
                if (ErrorMsg.Length == 0)
                {
                    ...
                    if (ParID == EntityID)  // prevent case which causes endless recursion
                    {
                        ParID = 0;
                    }
                    if (!Editing)
                    {
                        ...
                        {
                            con.Open();
                            using (IDataReader rs = DB.GetRS("select " + specs.m_EntityName + "ID from " + specs.m_EntityName + "   with (NOLOCK)  where Deleted=0 and " + specs.m_EntityName + "GUID=" + DB.SQuote(NewGUID), con))
                            {
                                rs.Read();
