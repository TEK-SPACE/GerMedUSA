...
public class ReceiptXsltExtension : XSLTExtensionBase
{
    ...
    private void PreComputeLineItemIntrinsics(XPathNavigator nav)
    {
        ...
        while (lineItemNodeIterator.MoveNext())
        {
            ...
            int quantity = XmlCommon.XmlFieldNativeInt(lineItemNode, "Quantity");
            decimal price = XmlCommon.XmlFieldNativeDecimal(lineItemNode, "OrderedProductRegularPrice");
            decimal orderedExtendedPrice = XmlCommon.XmlFieldNativeDecimal(lineItemNode, "OrderedProductPrice");
            decimal taxRate = XmlCommon.XmlFieldNativeDecimal(lineItemNode, "TaxRate");

            decimal vatAmount = decimal.Zero;
            decimal extendedVatAmount = decimal.Zero;

            bool applyVat = AppLogic.ProductIsMLExpress() == false && AppLogic.AppConfigBool("VAT.Enabled") == true &&
            ...
            vatAmountNode.InnerText = XmlCommon.XmlEncode(vatAmount.ToString());

            XmlNode extVatAmountNode = lineItemNode.OwnerDocument.CreateNode(XmlNodeType.Element, "ExtVatAmount", string.Empty);
            extVatAmountNode.InnerText = XmlCommon.XmlEncode(extendedVatAmount.ToString());
            ...
            extRegularPriceNode.InnerText = XmlCommon.XmlEncode(regularExtendedPrice.ToString());
            // insert these nodes on the bottom
            ...
            discountNode.InnerText = XmlCommon.XmlEncode(discount.ToString());
            // insert these nodes on the bottom
            ...
            displayPriceNode.InnerText = XmlCommon.XmlEncode(displayPrice.ToString());
            // insert these nodes on the bottom
            ...
            displayExtPriceNode.InnerText = XmlCommon.XmlEncode(displayExtPrice.ToString());
            // insert these nodes on the bottom
        ...
        hasLineItemDiscountsNode.InnerText = XmlCommon.XmlEncode(hasLineItemDiscounts.ToString());
            ...
            if (!string.IsNullOrEmpty(orderOptions))
            {
                ...
                foreach (string orderOptionDelimitedValue in orderOptionDelimitedValues)
                {
                    ...
                    if (orderOptionValues != null && orderOptionValues.Length > 0)
                    {
                        ...
                        if (withVat)
                        {
                            vatFormatted = orderOptionValues[3];
    ...
    /// <param name="filler">The padding character string</param>
