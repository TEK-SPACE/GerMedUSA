    ...
    public class SkinBase : System.Web.UI.Page
    {
        ...
        public SkinBase(String TemplateName)
        {
            ...
            if (TemplateName.Length == 0)
            {
                ...
                {
                    m_TemplateName = "expressTemplate.ascx";
                }
                else
                {
        ...
        public void LoadSkinTemplate()
        {
            ...
            if (m_TemplateName.Length != 0)
            {
                ...
                if (SkinID == 0 && CommonLogic.QueryStringCanBeDangerousContent("AffiliateID").Length != 0)
                {
                    ...
                    {
                        con.Open();
                        using (IDataReader rs = DB.GetRS("Select DefaultSkinID from Affiliate   with (NOLOCK)  where AffiliateID=" + CommonLogic.QueryStringUSInt("AffiliateID").ToString(), con))
                        {
                            if (rs.Read())
                            {
                                SkinID = DB.RSFieldInt(rs, "DefaultSkinID");
                            }
                        }
                    }
                }
                ...
                if (_url == null)
                {
                    ...
                    if (!CommonLogic.FileExists(m_TemplateFN))
                    {
                        // try default skin with base locale:
