...
namespace AspDotNetStorefront
{
    ...
    public partial class secureprocess : System.Web.UI.Page
    {
        private void Page_Load(object sender, System.EventArgs e)
        {
            ...
            // Cardia Services special handling
            if (AppLogic.ActivePaymentGatewayCleaned() == Gateway.ro_GWCARDIA && paReq == Gateway.ro_GWCARDIA)
            {
                if (CommonLogic.QueryStringCanBeDangerousContent("3D") == AppLogic.ro_OK)
                {
                    MerchantData = ThisCustomer.ThisCustomerSession["3Dsecure.MD"]; // this just makes it so we pass the test below
                    PaRes = AppLogic.ro_3DSecure; // This will trigger Cardia to confirm the transaction status and proceed
                }
                else
                {
                    // failed
                    ReturnURL = "checkoutpayment.aspx?error=1&errormsg=" + Server.UrlEncode(AppLogic.GetString("secureprocess.aspx.3", 1, Localization.GetWebConfigLocale()));
                }
            }
