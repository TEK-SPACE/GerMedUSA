...
namespace AspDotNetStorefront
{
    ...
    public partial class checkoutshippingmult : SkinBase
    {
        ...
        private string WriteItemAddresses()
        {
            ...
            s.Append("function AddresscontainsPOBOX(SrcID)\n"); 
            s.Append("{\n");
            s.Append("var addr = stripCharsInBag(stripWhitespace(document.getElementById('AddressAddress1_' + SrcID).value),'.');\n");
            s.Append("if((addr.indexOf(\"pobox\") >= 0) || (addr.indexOf(\"box\") >= 0) || (addr.indexOf(\"postoffice\") >= 0))\n");
            s.Append("{\n");
            ...
            {
                if (!c.m_IsDownload && c.m_Shippable && !GiftCard.s_IsEmailGiftCard(c.m_ProductID) && c.m_SKU != AppLogic.ro_PMMicropay)
                {
                    for (int i = 1; i <= c.m_Quantity; i++)
                    {
                        ...
                        s.Append("	if(AddresscontainsPOBOX('" + ThisID + "'))\n");
                        s.Append("	{\n");
                        s.Append("	    alert(\"" + AppLogic.GetString("createaccount_process.aspx.3", SkinID, ThisCustomer.LocaleSetting) + "\");\n");
                        s.Append("		theForm.ContinueCheckout.value='0';\n");
                        s.Append("	    submitenabled(theForm);\n");
                        s.Append("	    return (false);\n");
                        s.Append("	}\n");
