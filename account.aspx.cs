...
namespace AspDotNetStorefront
{
    ...
    public partial class account : SkinBase
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            StoreCCRow.Visible = AppLogic.StoreCCInDB();
        ...
        public void RefreshPage()
        {
            ...
            {
                con.Open();
                using (IDataReader rs = DB.GetRS("Select OrderNumber, OrderDate, RecurringSubscriptionID, PaymentMethod, CardNumber, TransactionState, QuoteCheckout, ShippedOn, ShippedVIA, ShippingTrackingNumber, DownloadEMailSentOn, QuoteCheckout, PaymentMethod, OrderTotal, CouponType, isnull(CouponDiscountAmount, 0) CouponDiscountAmount, CustomerServiceNotes  from dbo.orders   with (NOLOCK)  where TransactionState in (" + String.Join(",", TrxStates) + ") and CustomerID=" + ThisCustomer.CustomerID.ToString() + " order by OrderDate desc", con))
                {
            ...
            if (CustPassword.Text == CustPassword2.Text)
            {
                try
                {
                    ...
                    if (AppLogic.AppConfigBool("UseStrongPwd") || ThisCustomer.IsAdminUser)
                    {
                        ...
                        if (Regex.IsMatch(CustPassword.Text, AppLogic.AppConfig("CustomerPwdValidator"), RegexOptions.Compiled))
                        {
                            args.IsValid = true;
                        }
                        else
