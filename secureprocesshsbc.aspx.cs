    ...
    public partial class secureprocessHSBC : System.Web.UI.Page
    {
        private void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (ReturnURL.Length == 0)
            {
                ...
                {
                    ReturnURL = "checkoutpayment.aspx?error=1&errormsg=" + Server.UrlEncode(status);
                }
            }

            if (ReturnURL.Length == 0)
            {
                ReturnURL = "checkoutpayment.aspx?error=1&errormsg=" + Server.UrlEncode(AppLogic.GetString("secureprocess.aspx.2", 1, Localization.GetWebConfigLocale()));
            }
