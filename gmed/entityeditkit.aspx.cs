    ...
    public partial class entityeditkit : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (CommonLogic.FormBool("IsSubmit"))
            {
                ...
                {
                    dbconn.Open();
                    using( IDataReader rsg = DB.GetRS("select * from KitGroup   with (NOLOCK)  where ProductID=" + ProductID.ToString(),dbconn))
                    {
                        while (rsg.Read())
                        {
        ...
        public void LoadContent()
        {
            ...
            tmpS.Append("<form enctype=\"multipart/form-data\" action=\"entityeditkit.aspx?productid=" + ProductID.ToString() + "&entityName=" + CommonLogic.QueryStringCanBeDangerousContent("entityName") + "&entityFilterID=" + CommonLogic.QueryStringCanBeDangerousContent("entityFilterID") + "\" method=\"post\" id=\"Form1\" name=\"Form1\" onReset=\"return confirm('Do you want to reset all fields to their starting values?');\">\n");

            KitProductData kitProduct = KitProductData.Find(ProductID, cust);
            if (kitProduct != null)
            {
                string errorMessage = string.Empty;
                if (!kitProduct.Validate(out errorMessage))
                {
                    tmpS.Append("<p align=\"left\"><b><font color=red>" + errorMessage + "</font></b></p>\n");
                }
            }
            ...
            {
                dbconn.Open();
                using(IDataReader rs = DB.GetRS("select * from KitGroup   with (NOLOCK)  where ProductID=" + ProductID.ToString() + " order by DisplayOrder,Name",dbconn))
                {
                    while (rs.Read())
                    {
                        tmpS.Append("<tr><td colspan=\"2\" bgcolor=\"" + CommonLogic.IIF(i % 2 == 0, "#FFFFFF", "#EEEEEE") + "\"><font style=\"font-size: 8pt; font-weight: bold\">Group: " + Security.HtmlEncode(DB.RSFieldByLocale(rs, "Name", cust.LocaleSetting)) + "</font></td></tr>");
                        ...
                        using (SqlConnection kitConn = new SqlConnection(DB.GetDBConn()))
                        {
                            kitConn.Open();
                            using (IDataReader rsst = DB.GetRS("select * from KitGroupType   with (NOLOCK)  order by DisplayOrder,Name", kitConn))
                            {
                                while (rsst.Read())
                                {
                        ...
                        tmpS.Append("&nbsp;&nbsp;\n");
                        tmpS.Append("<input class=\"normalButtons\" type=\"button\" value=\"Delete This Group\" name=\"DeleteGroup_" + ThisGroupID.ToString() + "\" onClick=\"DeleteGroup(" + ThisGroupID.ToString() + ")\">\n");
                        tmpS.Append("</td></tr>");
                        ...
                        {
                            conn.Open();
                            using( IDataReader rsi = DB.GetRS("select * from KitItem   with (NOLOCK)  where KitGroupID=" + ThisGroupID.ToString() + " order by DisplayOrder,Name",conn))
                            {
                                while (rsi.Read())
                                {
                                    tmpS.Append("<tr>");
                                    ...
                                    tmpS.Append("</td>");
                                    tmpS.Append("<td>");
                                    tmpS.Append("<input class=\"normalButtons\" type=\"button\" value=\"Delete This Item\" name=\"DeleteItem_" + DB.RSFieldInt(rsi, "KitItemID").ToString() + "\" onClick=\"DeleteItem(" + DB.RSFieldInt(rsi, "KitItemID").ToString() + ")\">\n");
                                    tmpS.Append("</td>");
                                    tmpS.Append("<tr>");
                                }
                            }
            ...
            {
                connect.Open();
                using(IDataReader rsst3 = DB.GetRS("select * from KitGroupType   with (NOLOCK)  order by DisplayOrder,Name",connect))
                {
                    while (rsst3.Read())
                    {
            ...
            tmpS.Append("<input type=\"submit\" value=\"Update\" name=\"submit\" class=\"normalButtons\">\n");
            tmpS.Append("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"reset\" value=\"Reset\" class=\"normalButtons\" name=\"reset\">\n");
            tmpS.Append("</td>\n");
            tmpS.Append("</tr>\n");
            tmpS.Append("</table>\n");
            tmpS.Append("</form>\n");
