            ...
            using (SqlConnection dbconn = DB.dbConn())
            {
                ...
                using (IDataReader rs = DB.GetRS("select * from PollAnswer   with (NOLOCK)  where PollAnswerID=" + PollAnswerID.ToString(), dbconn))
                {
                    ...
                    if (ErrorMsg.Length == 0)
                    {
                        ...
                        if (Editing)
                        {
                            writer.Write("<input type=\"submit\" value=\"Update\" name=\"submit\" class=\"normalButtons\">\n");
                            writer.Write("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"reset\"  class=\"normalButtons\" value=\"Reset\" name=\"reset\">\n");
                        }
                        else
                        {
                            writer.Write("<input type=\"submit\" value=\"Add New\" name=\"submit\" class=\"normalButtons\">\n");
                        }
                        writer.Write("        </td>\n");
                        writer.Write("      </tr>\n");
