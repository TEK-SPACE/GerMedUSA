    ...
    public partial class Affiliates : System.Web.UI.Page
    {
        ...
        protected void resultFilter(string SearchFor)
        {
            ...
            if (SearchFor.Length != 0)
            {
                ...
                WhereClause += " (Name like " + DB.SQuote("%" + SearchFor + "%") + " or LastName like " + DB.SQuote("%" + SearchFor + "%") + " or [Company] like " + DB.SQuote("%" + SearchFor + "%") + ")";
            }
        ...
        {
            ltNotice.Text = "There are " + DB.GetSqlN("select count(affiliateid) as N from affiliate   with (NOLOCK)  where deleted=0").ToString() + " affiliates in the system.";
            ...
            using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
            {
                ...
                using (IDataReader rs = DB.GetRS("select AffiliateID, Name, EMail, URL, TrackingOnly from Affiliate   with (NOLOCK)  where parentAffiliateid=" + ParentAffiliateID.ToString() + " and deleted=0 order by DisplayOrder,Name", con))
                {
                    while (rs.Read())
                    {
                        ...
                        DataRow drow = dt.NewRow();
                        drow[0] = affiliateID; // DB.RSFieldInt(rs, "AffiliateID");
                        drow[1] = Indent + name; // row[1];
                        drow[2] = email; // row[2].ToString();
                        drow[3] = url; // row[3];
                        drow[4] = trackingUrl; // row[4];
                        dt.Rows.Add(drow);
        ...
        {
            Response.Redirect("editAffiliates.aspx?iden=0");
        }
