            ...
            using (SqlConnection dbconn = DB.dbConn())
            {
                ...
                using (IDataReader rs = DB.GetRS("select * from Poll   with (NOLOCK)  where PollID=" + PollID.ToString(), dbconn))
                {
                    ...
                    if (ErrorMsg.Length == 0)
                    {
                        ...
                        {
                            writer.Write("<div style=\"height:17;padding-top:3px;\" class=\"tablenormal\">Adding New Poll:</div>\n");
                        }
                        ...
                        writer.Write("              <td align=\"left\" valign=\"top\"><input type=\"text\" name=\"ExpiresOn\" id=\"ExpiresOn\" size=\"11\" value=\"" + CommonLogic.IIF(Editing, Server.HtmlEncode(Localization.ToNativeShortDateString(DB.RSFieldDateTime(rs, "ExpiresOn"))), Localization.ToNativeShortDateString(System.DateTime.Now.AddMonths(1))) + "\">&nbsp;<img src=\"" + AppLogic.LocateImageURL("skins/skin_" + SkinID.ToString() + "/images/calendar.gif") + "\" style=\"cursor:hand;cursor:pointer;\" align=\"absmiddle\" id=\"f_trigger_s\">");
