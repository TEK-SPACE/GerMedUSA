...
public partial class entityEditProductVariant : Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            else
            {
                ...
                trRecurringIntervalDiv.Visible = false;
                rblRecurringIntervalType.Items.Clear();
                rblRecurringIntervalType.Items.Add(
            ...
            if (!UseSpecialRecurringIntervals)
            {
                ltNormalSubscription.Visible = true;
                ltSpecialSubscription.Visible = false;
                txtSubscriptionInterval.Visible = true;
                rblSubscription.Visible = false;
            ...
            {
                ltNormalSubscription.Visible = false;
                ltSpecialSubscription.Visible = true;
                txtSubscriptionInterval.Visible = false;
                rblSubscription.Visible = true;
            ...
            if (!AppLogic.m_ProductIsML() || AppLogic.ProductIsMLX() || AppLogic.ProductIsMLExpress())
            {
                ...
                trISDownload.Visible = false;
                trDownloadLoc.Visible = false;
            }
            else
            {
        ...
        using (SqlConnection dbconn = new SqlConnection(DB.GetDBConn()))
        {
            ...
            using( IDataReader rs = DB.GetRS("select PV.*,P.ColorOptionPrompt,P.SizeOptionPrompt from Product P  with (NOLOCK)  , productvariant PV   with (NOLOCK)  where PV.ProductID=P.ProductID and VariantID=" + vID.ToString(),dbconn))
            {
                if (rs.Read())
                {
                    ...
                    {
                        btnDeleteVariant.Text = AppLogic.GetString("admin.common.DeleteNA", skinID, ThisCustomer.LocaleSetting) + " <img onmouseover=\"ddrivetip('<font class=\\'exampleText\\'>" + AppLogic.GetString("admin.common.DeleteNAInfoRecurring", skinID, ThisCustomer.LocaleSetting) + "</font>', 300)\" src=\"images/info.gif\" border=\"0\" onmouseout=\"hideddrivetip()\" align=\"absmiddle\" alt=\"\" />";
                ...
                if (Editing)
                {
                    ...
                    ltEntity.Text = entity.GetEntityBreadcrumb6(eID, ThisCustomer.LocaleSetting) + " : <a href=\"entityEditProducts.aspx?iden=" + pID + "&entityName=" + eName + "&EntityFilterID=" + eID + "\">" + AppLogic.GetProductName(pID, ThisCustomer.LocaleSetting) + " (" + pID + ")</a> : <a href=\"entityProductVariantsOverview.aspx?ProductID=" + pID + "&entityname=" + eName + "&EntityID=" + eID + "\">Variant Management</a> : " + XmlCommon.GetLocaleEntry(DB.RSField(rs, "Name"), locale, false) + " (" + vID + ")";
                    ...
                    {
                        ltExtendedPricing.Text = "<a href=\"javascript:;\" onclick=\"window.open('entityEditExtendedPrices.aspx?ProductID=" + pID.ToString() + "&VariantID=" + vID.ToString() + "','Pricing','height=400, width=500, scrollbars=yes, resizable=yes, toolbar=no, status=yes, location=no, directories=no, menubar=no, alwaysRaised=yes');\">Define Extended Prices</a> (Defined By Customer Level)";
                    }
                    txtSalePrice.Text = CommonLogic.IIF(DB.RSFieldDecimal(rs, "SalePrice") != Decimal.Zero, Localization.DecimalStringForDB(DB.RSFieldDecimal(rs, "SalePrice")), "");
                    ...
                    txtMSRP.Text = CommonLogic.IIF(DB.RSFieldDecimal(rs, "MSRP") != Decimal.Zero, Localization.CurrencyStringForDBWithoutExchangeRate(DB.RSFieldDecimal(rs, "MSRP")), "");
                    txtActualCost.Text = CommonLogic.IIF(DB.RSFieldDecimal(rs, "Cost") != Decimal.Zero, Localization.CurrencyStringForDBWithoutExchangeRate(DB.RSFieldDecimal(rs, "Cost")), "");

                    rblTaxable.SelectedIndex = (DB.RSFieldBool(rs, "IsTaxable") ? 1 : 0);

                    //MOD START GS - Set requires shipping radio button to value from DB
                    ...
                    else
                    {
                        ...
                        try
                        {
                            rblSubscription.SelectedValue = DB.RSFieldInt(rs, "SubscriptionInterval").ToString();
                        }
                        catch { }
                    }
                    ...
                    // COLORS & SIZES:
                    txtColors.Text = XmlCommon.GetLocaleEntry(Server.HtmlDecode(DB.RSField(rs, "Colors")), locale, false);
                    txtColorSKUModifiers.Text = XmlCommon.GetLocaleEntry(DB.RSField(rs, "ColorSKUModifiers"),locale,false);
                    txtSizes.Text = XmlCommon.GetLocaleEntry(Server.HtmlDecode(DB.RSField(rs, "Sizes")), locale, false);
                    ...
                    if (Editing && (DB.RSField(rs, "Colors").Length != 0 || DB.RSField(rs, "ColorSKUModifiers").Length != 0 || DB.RSField(rs, "Sizes").Length != 0 || DB.RSField(rs, "SizeSKUModifiers").Length != 0))
                    {
                        ...
                        temp += ("<table border=\"1\" cellpadding=\"0\" cellspacing=\"0\"><tr valign=\"left\"><td colspan=\"2\" height=\"10\"></td></tr>\n");
                        temp += ("<tr valign=\"left\">\n");
                             ...
                             DB.RSField(rs, "ColorSKUModifiers").Length != 0)
                        {
                            ...
                            for (int i = 0; i <= ColorCols; i++)
                            {
                                ...
                                temp += ("<td align=\"center\">" + Server.HtmlEncode(ColorVal) + "</td>\n");
                            }
                            temp += ("<tr>\n");
                            temp += ("<tr>\n");
                            ...
                            for (int i = 0; i <= ColorCols; i++)
                            {
                                ...
                                temp += ("<td align=\"center\">" + Server.HtmlEncode(SKUVal) + "</td>\n");
                            }
                            temp += ("<tr>\n");
                            temp += ("</table>\n");
                            temp += ("<br/><br/>");
                             ...
                             DB.RSField(rs, "SizeSKUModifiers").Length != 0)
                        {
                            ...
                            for (int i = 0; i <= SizeCols; i++)
                            {
                                ...
                                temp += ("<td align=\"center\">" + Server.HtmlEncode(SizeVal) + "</td>\n");
                            }
                            temp += ("<tr>\n");
                            temp += ("<tr>\n");
                            temp += ("<td><b>SKU Modifier</b></td>\n");
    ...
    protected void UpdateVariant()
    {
        ...
        if (UseSpecialRecurringIntervals)
        {
            txtSubscriptionInterval.Text = rblSubscription.SelectedValue.ToString();
        }
        ...
        if (!Editing)
        {
            ...
            if (bUseHtmlEditor)
            {
                temp = AppLogic.FormLocaleXmlVariant("Description", radDescription.Html, locale, vID);
            }
            else
            {
            ...
            sql.Append(DB.SQuote(AppLogic.FormLocaleXmlVariant("Colors", txtColors.Text, locale, vID).Replace(", ", ",").Replace(" ,", ",").Replace("'", "").Trim()) + ",");
            sql.Append(DB.SQuote(txtColorSKUModifiers.Text.Replace(", ", ",").Replace(" ,", ",").Trim()) + ",");
            sql.Append(DB.SQuote(AppLogic.FormLocaleXmlVariant("Sizes", txtSizes.Text, locale, vID).Replace(", ", ",").Replace(" ,", ",").Replace("'", "").Trim()) + ",");
        ...
        else
        {
            ...
            sql.Append("Inventory=" + CommonLogic.IIF(txtCurrentInventory.Text.Length != 0, txtCurrentInventory.Text, "1000000") + ",");
            ...
            sql.Append("Colors=" + DB.SQuote(AppLogic.FormLocaleXmlVariant("Colors", 
                XmlCommon.GetLocaleEntry(Security.HtmlEncode(txtColors.Text),locale,false), 
            ...
            sql.Append("Sizes=" + DB.SQuote(AppLogic.FormLocaleXmlVariant("Sizes", 
                XmlCommon.GetLocaleEntry(Security.HtmlEncode(txtSizes.Text),locale,false), 
                locale, vID).Replace(", ", ",").Replace(" ,", ",").Replace("'", "").Trim()) + ",");
