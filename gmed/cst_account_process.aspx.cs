    ...
    public partial class cst_account_process : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (CommonLogic.QueryStringUSInt("resetpwd") > 0)
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        String vtr = CommonLogic.FormCanBeDangerousContent("VATRegistrationID").Trim();
                        if (vtr.Length > 0 && (!AppLogic.AppConfigBool("VAT.Enabled") || !AppLogic.VATRegistrationIDIsValid(TargetCustomer, vtr)))
                        {
                            vtr = null;
                        }
            ...
            if (CommonLogic.FormCanBeDangerousContent("ForcePassword").Trim().Length > 0)
            {
                ...
                {
                    if (!Regex.IsMatch(CommonLogic.FormCanBeDangerousContent("ForcePassword").Trim(), AppLogic.AppConfig("CustomerPwdValidator"), RegexOptions.Compiled))
                    {
            ...
            if (CommonLogic.FormCanBeDangerousContent("CouponCode").Length != 0)
            {
                ...
                {
                    dbconn.Open();
                    using (IDataReader rsdis = DB.GetRS("select * from Coupon   with (NOLOCK)  where lower(CouponCode)=" + DB.SQuote(CommonLogic.FormCanBeDangerousContent("CouponCode").ToLowerInvariant().Trim()) + " and expirationdate>=getdate() and deleted=0", dbconn))
                    {
                        if (rsdis.Read())
                        {
            ...
            {
                String vtr = CommonLogic.FormCanBeDangerousContent("VATRegistrationID").Trim();
                if (vtr.Length > 0 && !AppLogic.VATRegistrationIDIsValid(TargetCustomer, vtr))
                {
                    vtr = String.Empty;
                    VATIsValid = false;
                }
            ...
            {
                // to prevent users from setting invalid flags own their super admin account:
                sql.Append("Active=1,");
                sql.Append("Over13Checked=1,");
                sql.Append("AdminCanViewCC=1,");
                sql.Append("OKToEMail=1,");
                sql.Append("CODCompanyCheckAllowed=1,");
                sql.Append("CODNet30Allowed=1,");
            }
