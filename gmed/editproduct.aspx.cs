...
namespace AspDotNetStorefrontAdmin
{
    ...
    public partial class editproduct : AspDotNetStorefront.SkinBase
    {
        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            NumCats = DB.GetSqlN("Select count(*) as N from category   with (NOLOCK)  where deleted=0");
            NumSecs = DB.GetSqlN("Select count(*) as N from [section]   with (NOLOCK)  where deleted=0");
            NumAffs = DB.GetSqlN("Select count(*) as N from affiliate   with (NOLOCK)  where deleted=0");
            NumCL = DB.GetSqlN("Select count(*) as N from customerlevel   with (NOLOCK)  where deleted=0");
            NumGenre = DB.GetSqlN("Select count(*) as N from genre   with (NOLOCK)  where deleted=0");
            NumVector = DB.GetSqlN("Select count(*) as N from Vector   with (NOLOCK)  where deleted=0");
            ...
            HasRecurringOrder = 0 < DB.GetSqlN("select count(*) N from ShoppingCart  with (NOLOCK)  where ProductID=" + ProductID.ToString() + " and CartType=" + ((int)CartTypeEnum.RecurringCart).ToString());
            ...
            if (CommonLogic.FormBool("IsSubmit"))
            {
                ...
                if (!Editing)
                {
                    ...
                    sql.Append("insert into product(ProductGUID,Name,SEName,ContentsBGColor,PageBGColor,GraphicsColor,AvailableStartDate,AvailableStopDate,ImageFilenameOverride,ProductTypeID,TaxClassID,Summary,Description,ExtensionData,ColorOptionPrompt,SizeOptionPrompt,RequiresTextOption,TextOptionPrompt,TextOptionMaxLength,FroogleDescription,RelatedProducts,UpsellProducts,UpsellProductDiscountPercentage,RequiresProducts,SEKeywords,SEDescription,SETitle,SENoScript,SEAltText,SKU,PageSize,ColWidth,XmlPackage,ManufacturerPartNumber,SalesPromptID,SpecTitle,SpecCall,Published,GoogleCheckoutAllowed,ShowBuyButton,IsCallToOrder,HidePriceUntilCart,ShowInProductBrowser,ExcludeFromPriceFeeds,IsAKit,IsAPack,PackSize,TrackInventoryBySizeAndColor,TrackInventoryBySize,TrackInventoryByColor,RequiresRegistration,SpecsInline,MiscText,SwatchImageMap,QuantityDiscountID) values(");
                    ...
                    {
                        sql.Append(DB.SQuote(Localization.ToDBShortDateString(Localization.ParseNativeDateTime(CommonLogic.FormCanBeDangerousContent("AvailableStartDate")))) + ",");
                    }
                    else
                    {
                        sql.Append("getdate(),");
                    ...
                    {
                        sql.Append(DB.SQuote(Localization.ToDBShortDateString(Localization.ParseNativeDateTime(CommonLogic.FormCanBeDangerousContent("AvailableStopDate")))) + ",");
                    }
                    else
                    {
                        sql.Append("NULL,");
                    ...
                    if (CommonLogic.FormCanBeDangerousContent("Price").Length != 0)
                    {
                        ...
                        sql.Append(CommonLogic.IIF(CommonLogic.FormCanBeDangerousContent("Inventory").Length != 0, CommonLogic.FormCanBeDangerousContent("Inventory"), CommonLogic.IIF(CommonLogic.IsInteger(AppLogic.AppConfig("Admin_DefaultInventory")), AppLogic.AppConfig("Admin_DefaultInventory"), "100000")) + ",");
                        sql.Append("1,");
                ...
                else
                {
                    ...
                    if ((CommonLogic.FormCanBeDangerousContent("AvailableStartDate").Length != 0) && (CommonLogic.IsValidDate(CommonLogic.FormCanBeDangerousContent("AvailableStartDate")) == true))
                    {
                    ...
                    else if ((CommonLogic.FormCanBeDangerousContent("AvailableStartDate").Length != 0) && (CommonLogic.IsValidDate(CommonLogic.FormCanBeDangerousContent("AvailableStartDate")) == false))
                    {
                        sql.Append("AvailableStartDate=AvailableStartDate,");
                    }
                    else
                    {
                    ...
                    if ((CommonLogic.FormCanBeDangerousContent("AvailableStopDate").Length != 0) && (CommonLogic.IsValidDate(CommonLogic.FormCanBeDangerousContent("AvailableStopDate")) == true))
                    {
                    ...
                    else if ((CommonLogic.FormCanBeDangerousContent("AvailableStopDate").Length != 0) && (CommonLogic.IsValidDate(CommonLogic.FormCanBeDangerousContent("AvailableStopDate")) == false))
                    {
                        sql.Append("AvailableStopDate=AvailableStopDate,");
                    }
                    else
                    {
        ...
        protected override void RenderContents(System.Web.UI.HtmlTextWriter writer)
        {
            ...
            {
                dbconn.Open();
                using (IDataReader rs = DB.GetRS("select * from Product   with (NOLOCK)  where ProductID=" + ProductID.ToString(), dbconn))
                {
                    if (rs.Read())
                    {
                        Editing = true;
                    ...
                    if (ErrorMsg.Length == 0)
                    {
                        ...
                        if (Editing)
                        {
                            ...
                            writer.Write("</p>\n");
                            writer.Write("<input class=\"normalButtons\" style=\"font-size: 9px;\" type=\"button\" value=\"Show/Edit Variants\" name=\"ShowVariants_" + ProductID.ToString() + "\" onClick=\"self.location='variants.aspx?productid=" + ProductID.ToString() + "'\">");
                            if (!DB.RSFieldBool(rs, "IsSystem"))
                            {
                                if (!HasRecurringOrder)
                                {
                                    writer.Write("&nbsp;&nbsp;");
                                    writer.Write("<input class=\"normalButtons\" style=\"font-size: 9px;\" type=\"button\" value=\"Delete All Variants\" name=\"DeleteAllVariants_" + ProductID.ToString() + "\" onClick=\"if(confirm('Are you sure you want to delete ALL variants for this product? This will also remove all items from customer shopping carts, wish lists, and gift registries IF they reference any of these variants!')){self.location='editproduct.aspx?DeleteAllVariants=true&productid=" + ProductID.ToString() + "';}\">");
                                }
                                
                                if (!AppLogic.ProductIsMLExpress() || !AppLogic.MaxProductsExceeded())
                                {
                                    writer.Write("&nbsp;&nbsp;");
                                    writer.Write("<input class=\"normalButtons\" style=\"font-size: 9px;\" type=\"button\" value=\"Clone Product\" name=\"Clone_" + ProductID.ToString() + "\" onClick=\"if(confirm('Are you sure you want to create a clone of this product, and all variants?')) {self.location='cloneproduct.aspx?productid=" + ProductID.ToString() + "';}\">\n");
                                }
                            }
                            if (!DB.RSFieldBool(rs, "Deleted"))
                            {
                                if (HasRecurringOrder)
                                {
                                    writer.Write("&nbsp;&nbsp;<input class=\"normalButtons\" disabled style=\"font-size: 9px;\" type='button' value='" + AppLogic.GetString("admin.common.DeleteNA", SkinID, ThisCustomer.LocaleSetting) + "' /> ");
                                    writer.Write("</b>" + AppLogic.GetString("admin.common.DeleteNAInfoRecurring", SkinID, ThisCustomer.LocaleSetting) + "<b>");
                                }
                                else
                                {
                                    writer.Write("&nbsp;&nbsp;<input class=\"normalButtons\" style=\"font-size: 9px;\" type='button' value='Delete Product' onclick=\"if(confirm('Are you sure you want to delete this product?')) {self.location='products.aspx?DeleteID=" + ProductID.ToString() + "'}\" />");
                                }
                            }
                            if (ProductTracksInventoryBySizeAndColor)
                            {
                                writer.Write("&nbsp;&nbsp;");
                                writer.Write("<input class=\"normalButtons\" style=\"font-size: 9px;\" type=\"button\" value=\"Edit Inventory\" name=\"EditInventory_" + ProductID.ToString() + "\"  onClick=\"self.location='editinventory.aspx?productid=" + ProductID.ToString() + "'\">");
                            }
                            writer.Write("</b>");
                        }
                        else
                        {
                            writer.Write("<div style=\"height:17;padding-top:3px;\" class=\"tablenormal\">Adding New Product:</div>\n");
                        }
                        ...
                        writer.Write("}\n");
                        writer.Write("\n");
                        writer.Write("function getkey(e)\n");
                        writer.Write("{\n");
                        writer.Write("if (window.event)\n");
                        writer.Write("return window.event.keyCode;\n");
                        writer.Write("else if (e)\n");
                        writer.Write("return e.which;\n");
                        writer.Write("else\n");
                        writer.Write("return null;\n");
                        writer.Write("}\n");
                        writer.Write("\n");
                        writer.Write("function Date_Validator(e,chars)\n");
                        writer.Write("{\n");
                        writer.Write("var characters = chars;\n");
                        writer.Write("var key, keychar;\n");
                        writer.Write("key = getkey(e);\n");
                        writer.Write("if (key == null) return true;\n");
                        writer.Write("keychar = String.fromCharCode(key);\n");
                        writer.Write("keychar = keychar.toLowerCase();\n");
                        writer.Write("characters = characters.toLowerCase();\n");
                        writer.Write("if (characters.indexOf(keychar) != -1)\n");
                        writer.Write("  {\n");
                        writer.Write("return (true);\n");
                        writer.Write("  }\n");
                        writer.Write("else\n");
                        ...
                        writer.Write("}\n");
                        writer.Write("\n");
                        writer.Write("function PackSizeCheck()\n");
                        writer.Write("{\n");
                        writer.Write("if (document.getElementById(\"IsAPack\").checked==true)\n");
                        writer.Write("  {\n");
                        writer.Write("if (document.getElementById(\"PackSize\").value==0)\n");
                        writer.Write("      {\n");
                        writer.Write("          alert(\"Setting pack size to 0 will result to 0 subtotal for this product when added to cart. Hence, this will not be considered as a pack.\");\n");
                        writer.Write("          document.getElementById(\"IsAPack\").checked=false\n");
                        writer.Write("      }\n");
                        writer.Write("  }\n");
                        writer.Write("}\n");
                        writer.Write("</script>\n");
                        ...
                        {
                            dbconn2.Open();
                            using (IDataReader rsst = DB.GetRS(prodTypeSQL, dbconn2))
                            {
                                while (rsst.Read())
                                {
                        ...
                        {
                            dbconn3.Open();
                            using (IDataReader rsst = DB.GetRS("select * from TaxClass  with (NOLOCK)  order by DisplayOrder,Name", dbconn3))
                            {
                                while (rsst.Read())
                                {
                        ...
                        {
                            dbconn4.Open();
                            using (IDataReader rsst = DB.GetRS("select * from Manufacturer  with (NOLOCK)  where deleted=0 order by DisplayOrder,Name", dbconn4))
                            {
                                while (rsst.Read())
                                {
                        ...
                        if (!AppLogic.ProductIsMLExpress())
                        {
                            ...
                            {
                                dbconn5.Open();
                                using (IDataReader rsst = DB.GetRS("select * from Distributor   with (NOLOCK)  where Deleted=0 order by DisplayOrder,Name", dbconn5))
                                {
                                    while (rsst.Read())
                                    {
                        ...
                        writer.Write("                <td align=\"left\" valign=\"top\">\n");
                        writer.Write("                	<input maxLength=\"100\" size=\"15\" id=\"AvailableStopDate\" onkeypress=\"return Date_Validator(event,'1234567890/')\" name=\"AvailableStopDate\" value=\"" + CommonLogic.IIF(Editing, Localization.ToNativeShortDateString(DB.RSFieldDateTime(rs, "AvailableStopDate")), "") + "\">&nbsp;<img src=\"" + AppLogic.LocateImageURL("skins/skin_" + SkinID.ToString() + "/images/calendar.gif") + "\" style=\"cursor:hand;cursor:pointer;\" align=\"absmiddle\" id=\"f_trigger_e\">&nbsp;<small>(" + Localization.ShortDateFormat() + ", leave blank to never expire the product)</small>\n");
                        writer.Write("                	</td>\n");
                        writer.Write("              </tr>\n");
                        ...
                        foreach (String s in xmlPackages)
                        {
                            ...
                            {
                                if (!s.ToLowerInvariant().Contains("pack") && !s.ToLowerInvariant().Contains("kit") && !s.ToLowerInvariant().Contains("tabbedui"))
                                {
                                    writer.Write("<option value=\"" + s + "\"");
                                    if (Editing)
                                    {
                        ...
                        using (SqlConnection dbconn6 = DB.dbConn())
                        {
                            dbconn6.Open();
                            using (IDataReader rsst = DB.GetRS("select * from QuantityDiscount   with (NOLOCK)  order by DisplayOrder,Name", dbconn6))
                            {
                                while (rsst.Read())
                                {
                        ...
                        writer.Write("              <tr valign=\"middle\">\n");
                        writer.Write("                <td align=\"right\" valign=\"middle\">*Exclude From Product Feeds:&nbsp;&nbsp;</td>\n");
                        writer.Write("                <td align=\"left\" >\n");
                        ...
                        if (!AppLogic.ProductIsMLX() && !AppLogic.ProductIsMLExpress())
                        {
                            ...
                            writer.Write("                <td align=\"left\" valign=\"top\">\n");
                            writer.Write("                	<input maxLength=\"10\" size=\"10\" name=\"PackSize\" onkeypress=\"return Date_Validator(event,'1234567890')\" value=\"" + CommonLogic.IIF(Editing, DB.RSFieldInt(rs, "PackSize").ToString(), "") + "\">\n");
                        ...
                        if (Editing && pdesc.Contents.Length != 0)
                        {
                            ...
                            writer.Write("<input class=\"normalButtons\" type=\"button\" value=\"Delete\" name=\"DeleteDescriptionFile_" + ProductID.ToString() + "\" onClick=\"DeleteDescriptionFile()\">");
                        ...
                        writer.Write("              <tr valign=\"middle\">\n");
                        writer.Write("                <td align=\"right\" valign=\"top\">Feed Description (NO HTML):&nbsp;&nbsp;</td>\n");
                        writer.Write("                <td align=\"left\" valign=\"top\">\n");
                        ...
                        if (AppLogic.ProductIsMLExpress())
                        {
                            Array.Clear(names, 2, 4);
                            Array.Clear(values, 2, 4);
                        }
                        ...
                        writer.Write("              <tr valign=\"middle\">\n");
                        writer.Write("                <td align=\"right\" valign=\"top\">Alt Text:&nbsp;&nbsp;</td>\n");
                        writer.Write("                <td align=\"left\" valign=\"top\">\n");
                        writer.Write(AppLogic.GetLocaleEntryFields(DB.RSField(rs, "SEAltText"), "SEAltText", true, true, false, "", 255, 100, 0, 0, false));
                        ...
                        using (SqlConnection dbconn7 = DB.dbConn())
                        {
                            dbconn7.Open();
                            using (IDataReader rsst = DB.GetRS("select * from salesprompt   with (NOLOCK)  where deleted=0", dbconn7))
                            {
                                while (rsst.Read())
                                {
                        ...
                        if (Editing && pspec.Contents.Length != 0)
                        {
                            ...
                            writer.Write("<input class=\"normalButtons\" type=\"button\" value=\"Delete\" name=\"DeleteSpecFile_" + ProductID.ToString() + "\" onClick=\"DeleteSpecFile()\">");
                            writer.Write("<div style=\"border-style: dashed; border-width: 1px;\">\n");
