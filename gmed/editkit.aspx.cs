    ...
    public partial class editkit : AspDotNetStorefront.SkinBase
    {
        ...
        protected override void RenderContents(System.Web.UI.HtmlTextWriter writer)
        {
            ...
            if (ErrorMsg.Length == 0)
            {
                ...
                KitProductData kitProduct = KitProductData.Find(ProductID, ThisCustomer);
                if (kitProduct != null)
                {
                    string errorMessage = string.Empty;
                    if (!kitProduct.Validate(out errorMessage))
                    {
                        writer.Write("<p align=\"left\"><b><font color=red>" + errorMessage + "</font></b></p>\n");
                    }
                }

                writer.Write("<form enctype=\"multipart/form-data\" action=\"editkit.aspx?productid=" + ProductID.ToString() + "\" method=\"post\" id=\"Form1\" name=\"Form1\" onReset=\"return confirm('Do you want to reset all fields to their starting values?');\">\n");
                ...
                writer.Write("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input class=\"normalButtons\" type=\"reset\" value=\"Reset\" name=\"reset\">\n");
                writer.Write("</td>\n");
                writer.Write("</tr>\n");
                int i = 1;
                ...
                using (SqlConnection dbconn = DB.dbConn())
                {
                    ...
                    {                        
                        while (rs.Read())
                        {
                            ...
                            {
                                dbconn2.Open();
                                using (IDataReader rsst = DB.GetRS("select * from KitGroupType   with (NOLOCK)  order by DisplayOrder,Name", dbconn2))
                                {
                                    while (rsst.Read())
                                    {
                            ...
                            writer.Write("&nbsp;&nbsp;\n");
                            writer.Write("<input class=\"normalButtons\" type=\"button\" value=\"Delete This Group\" name=\"DeleteGroup_" + ThisGroupID.ToString() + "\" onClick=\"DeleteGroup(" + ThisGroupID.ToString() + ")\">\n");
                            writer.Write("</td></tr>");
                            ...
                            {
                                dbconn3.Open();
                                using (IDataReader rsi = DB.GetRS("select * from KitItem   with (NOLOCK)  where KitGroupID=" + ThisGroupID.ToString() + " order by DisplayOrder,Name", dbconn3))
                                {
                                    while (rsi.Read())
                                    {
                                        writer.Write("<tr>");
                                        ...
                                        writer.Write("</td>");
                                        writer.Write("<td>");
                                        writer.Write("<input class=\"normalButtons\" type=\"button\" value=\"Delete This Item\" name=\"DeleteItem_" + DB.RSFieldInt(rsi, "KitItemID").ToString() + "\" onClick=\"DeleteItem(" + DB.RSFieldInt(rsi, "KitItemID").ToString() + ")\">\n");
                                        writer.Write("</td>");
                                        writer.Write("<tr>");
                                    }
                                }
                ...
                {
                    dbconn4.Open();
                    using (IDataReader rsst3 = DB.GetRS("select * from KitGroupType   with (NOLOCK)  order by DisplayOrder,Name", dbconn4))
                    {
                        while (rsst3.Read())
                        {
