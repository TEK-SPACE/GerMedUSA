            ...
            if (CommonLogic.FormBool("IsSubmit"))
            {
                        ...
                        if (FieldName.IndexOf("Key_") != -1)
                        {
                            ...
                            if (Image2File.ContentLength != 0)
                            {
                                ...
                                if (TheSize == "large")
                                {
                                    ...
                                    bool globalCreate = (AppLogic.AppConfigBool("LargeCreatesOthers") && !AppLogic.ProductIsMLExpress());
                                    bool globalOverwrite = (AppLogic.AppConfigBool("LargeOverwritesOthers") && !AppLogic.ProductIsMLExpress());

                                    String localCreate = String.Empty;
                                    String localOverwrite = String.Empty;

                                    Hashtable configValues = AppLogic.SplitConfig("Product", "large");
                                    ...
                                    if (largeCreates)
                                    {
                                        if (largeOverwrites)
                                        {
                                            if (SafeColor == "")
                                            {
                                                try
                                                {
                                                    ...
                                                    {
                                                        System.IO.File.Delete(AppLogic.GetImagePath("Product", "micro", true) + FN + "_" + ImageNumber.ToLowerInvariant() + "_" + SafeColor + ss);
                                                    }
                                                }
                                                catch { }
                                        ...
                                        else
                                        {
                                            ...
                                            foreach (String ss in CommonLogic.SupportedImageTypes)
                                            {
                                                if (CommonLogic.FileExists(AppLogic.GetImagePath("Product", "micro", true) + FN + ss))
                                                    microExists = true;
                                            }
                ...
                using (SqlConnection dbconn = new SqlConnection(DB.GetDBConn()))
                {
                    ...
                    using(  IDataReader rsColors = DB.GetRS("select Colors from productvariant   with (NOLOCK)  where VariantID=" + VariantID.ToString(),dbconn))
                    {
                        if (rsColors.Read())
                        {
                            variantColors = DB.RSFieldByLocale(rsColors, "Colors", Localization.GetWebConfigLocale());
                        }
                    }
                }
                if (AppLogic.AppConfigBool("MultiColorMakesSwatchAndMap") && variantColors.Length > 0 && TheSize.ToUpperInvariant() != "ICON")
                {
                    AppLogic.MakeColorSwatch(ProductID,skinID,ThisCustomer.LocaleSetting,variantColors,FN);
                }
                ...
                using(IDataReader rs = DB.GetRS("select * from productvariant   with (NOLOCK)  where VariantID=" + VariantID.ToString(),dbconn))
                {
                    ...
                    for (int i = ColorsSplit.GetLowerBound(0); i <= ColorsSplit.GetUpperBound(0); i++)
                    {
                        ...
                        for (int j = ImageNumbersSplit.GetLowerBound(0); j <= ImageNumbersSplit.GetUpperBound(0); j++)
                        {
                            ...
                            temp += ("<img " + CommonLogic.IIF(ImgWidth != 0, "width=\"" + ImgWidth.ToString() + "\"", "") + " src=\"" + AppLogic.LookupProductImageByNumberAndColor(ProductID, skinID, ThisCustomer.LocaleSetting, Localization.ParseUSInt(ImageNumbersSplit[j]), AppLogic.CleanSizeColorOption(ColorsSplit[i]), TheSize) + "\"><br/>");
