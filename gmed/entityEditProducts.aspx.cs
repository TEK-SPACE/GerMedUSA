...
public partial class entityEditProducts : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        {
            lblCustLevels.Visible = false;
            lblAffiliate.Visible = false;
            trDistributor.Visible = false;
            trGC.Visible = false;
        ...
        if (!IsPostBack)
        {
            ...
            {
                trIsAPack.Visible = false;
                trAllowAddToPack.Visible = false;
            }
            else
            {
                if (!AppLogic.ProductIsMLExpress())
        ...
        using (SqlConnection dbconn = new SqlConnection(DB.GetDBConn()))
        {
            ...
            using (IDataReader rs = DB.GetRS("select * from Product   with (NOLOCK)  where ProductID=" + pID.ToString(), dbconn))
            {
                if (rs.Read())
                {
                    ...
                    {
                        btnDeleteProduct.Text = AppLogic.GetString("admin.common.DeleteNA", skinID, ThisCustomer.LocaleSetting)
                            + " <img onmouseover=\"ddrivetip('<font class=\\'exampleText\\'>"
                            + AppLogic.GetString("admin.common.DeleteNAInfoRecurring", skinID, ThisCustomer.LocaleSetting)
                            + "</font>', 300)\" src=\"images/info.gif\" border=\"0\" onmouseout=\"hideddrivetip()\" align=\"absmiddle\" alt=\"\" />";
                ...
                {
                    query = "select * from producttype   with (NOLOCK) where [name] not like '%pack%' order by DisplayOrder,Name";
                }
                else
                {
                    if (AppLogic.ProductIsMLExpress())
                    {
                        query = "select * from producttype  with (NOLOCK) where [name] not like '%pack%' and [name] not like '%kit%'  and [name] not like '%gift card%' order by DisplayOrder,Name";                        
                    }
                    else
                    {
                ...
                string sql = "select * from Manufacturer  with (NOLOCK)  where deleted=0 and published = 1 order by DisplayOrder,Name";
                ...
                if (AppLogic.ProductIsMLExpress())
                {
                    ...
                    int manufacturersAllowedToBeDisplayed = 0;
                    ...
                    using (SqlConnection conn = new SqlConnection(DB.GetDBConn()))
                    {
                        ...
                        manufacturersAllowedToBeDisplayed = AppLogic.MLEXPRESS_MAX_ALLOWED_ENTITIES - (entityCategoryCount + entitySectionCount);
                    }
                    sql = string.Format("select Top {0} * from Manufacturer  with (NOLOCK)  where deleted=0 and published = 1 order by DisplayOrder,Name", manufacturersAllowedToBeDisplayed);
                }
                ...
                {
                    if (!AppLogic.ProductIsMLX()) // if not incartia
                    {
                        if (!AppLogic.ProductIsMLExpress())
                        {
                    ...
                    {
                        if (!s.ToLowerInvariant().Contains("pack"))
                        {
                            ddXmlPackage.Items.Add(new ListItem(s, s));
                        }
                    }
                ...
                if (Editing)
                {
                    ...
                    ltEntity.Text = entity.GetEntityBreadcrumb6(eID, ThisCustomer.LocaleSetting) + " : <a href=\"entityProducts.aspx?entityName=" + eName + "&EntityFilterID=" + eID + "\">Product Management</a> : " + XmlCommon.GetLocaleEntry(DB.RSField(rs, "Name"), locale, false) + " (" + pID + ")";
                    ...
                    if (bUseHtmlEditor)
                    {
                        radDescription.Html = XmlCommon.GetLocaleEntry(DB.RSField(rs, "Description"), locale, false);
                    }
                    else
                    {
                    ...
                    if (bUseHtmlEditor)
                    {
                        radSummary.Html = XmlCommon.GetLocaleEntry(DB.RSField(rs, "Summary"), locale, false);
                    }
                    else
                    {
                    ...
                    // Youtube Video Link
                    txtVideoURL.Text = DB.RSField(rs, "VideoURL").Replace("https://www.youtube.com/embed/","");

                    // Audio File link
                    txtAudioURL.Text = DB.RSField(rs, "AudioURL").Replace("http://yourlisten.com/swf/Player.swf?id=","");
    ...
    protected void UpdateProduct()
    {
        ...
        if (!Editing)
        {
            ...
            sql.Append("insert into product(ProductGUID,Name,SEName,ContentsBGColor,PageBGColor,GraphicsColor,ImageFilenameOverride,ProductTypeID,Summary,Description,ExtensionData,ColorOptionPrompt,SizeOptionPrompt,RequiresTextOption,TextOptionPrompt,TextOptionMaxLength,FroogleDescription,RelatedProducts,UpsellProducts,UpsellProductDiscountPercentage,RequiresProducts,SEKeywords,SEDescription,SETitle,SENoScript,SEAltText,SKU,PageSize,ColWidth,XmlPackage,ManufacturerPartNumber,SalesPromptID,SpecTitle,SpecCall,Published,GoogleCheckoutAllowed,ShowBuyButton,IsCallToOrder,HidePriceUntilCart,ShowInProductBrowser,ExcludeFromPriceFeeds,IsAKit,IsAPack,PackSize" + CommonLogic.IIF(AppLogic.AppConfigBool("ShowPopularity"), ",Popularity", "") + ",TrackInventoryBySizeAndColor,TrackInventoryBySize,TrackInventoryByColor,RequiresRegistration,SpecsInline,MiscText,SwatchImageMap,QuantityDiscountID,TaxClassID,AvailableStartDate,AvailableStopDate,VideoURL,AudioURL) values(");
            ...
            if (bUseHtmlEditor)
            {
                sql.Append(DB.SQuote(AppLogic.FormLocaleXml("Summary", radSummary.Html, locale, "product", eID)) + ",");
                sql.Append(DB.SQuote(AppLogic.FormLocaleXml("Description", radDescription.Html, locale, "product", eID)) + ",");
            }
            else
            {
            ...
            if (!string.IsNullOrEmpty(txtVideoURL.Text))
                sql.Append("https://www.youtube.com/embed/" + txtVideoURL.Text.Trim() + ",");
				else
				 sql.Append("NULL,");
            if (!string.IsNullOrEmpty(txtAudioURL.Text))
                sql.Append("http://yourlisten.com/swf/Player.swf?id=" + txtAudioURL.Text.Trim() );
				else
				 sql.Append("NULL");
            sql.Append(")");
            ...
            if (txtPrice.Text.Length != 0)
            {
                ...
                sql.Append(CommonLogic.IIF(txtInventory.Text.Length != 0, Localization.ParseNativeInt(txtInventory.Text).ToString(), CommonLogic.IIF(CommonLogic.IsInteger(AppLogic.AppConfig("Admin_DefaultInventory")), AppLogic.AppConfig("Admin_DefaultInventory"), "100000")) + ",");
                sql.Append("1,");
        ...
        else
        {
            ...
            if (!string.IsNullOrEmpty(txtVideoURL.Text))
                sql.Append("VideoURL=N'https://www.youtube.com/embed/" + txtVideoURL.Text.Trim() + "',");
            if (!string.IsNullOrEmpty(txtAudioURL.Text))
                sql.Append("AudioURL=N'http://yourlisten.com/swf/Player.swf?id=" + txtAudioURL.Text.Trim() + "',");
            if (bUseHtmlEditor)
            {
                sql.Append("Summary=" + DB.SQuote(AppLogic.FormLocaleXml("Summary", radSummary.Html, locale, "product", pID)) + ",");
                sql.Append("Description=" + DB.SQuote(AppLogic.FormLocaleXml("Description", radDescription.Html, locale, "product", pID)) + ",");
            ...
            else
            {
                ...
                {
                    conn.Open();
                    using (IDataReader reader = DB.GetRS("select SwatchImageMap from Product  with (NOLOCK)  where ProductID=" + pID.ToString(), conn))
                    {
                        if (reader.Read())
                        {
                            sql.Append("SwatchImageMap=" + DB.SQuote(DB.RSField(reader, "SwatchImageMap")) + ",");
                        }
                        else
                        {
    ...
    public void ValidateDateRange(object source, ServerValidateEventArgs args)
    {
        ...
        {
            DateTime sdate = Localization.ParseLocaleDateTime(txtStartDate.Text, Localization.GetWebConfigLocale());
            DateTime edate = Localization.ParseLocaleDateTime(txtStopDate.Text, Localization.GetWebConfigLocale());
