...
namespace AspDotNetStorefrontAdmin
{
    ...
    public partial class showcustomerlevel : AspDotNetStorefront.SkinBase
	{
		...
		{
            writer.Write("<p><b>There are " + DB.GetSqlN("select count(CustomerID) as N from Customer  with (NOLOCK)  where EMail <> '' and Deleted=0 and CustomerLevelID=" + CustomerLevelID.ToString()).ToString() + " registered customers in this customer level</b></p>");
            ...
            writer.Write("&nbsp;&nbsp;Search For: <input type=\"text\" name=\"SearchFor\" value=\"" + SearchFor + "\"><input type=\"submit\" name=\"search\" value=\"submit\" class=\"normalButtons\">");
            writer.Write("</form>");
            ...
            using (DataTable dt = new DataTable())
            {
                ...
                for (int i = 0; i < rowcount; i++)
                {
                    ...
                    writer.Write("<td width=\"10%\" align=\"center\"><input type=\"button\" value=\"Clear Level\" name=\"Delete_" + DB.RowFieldInt(row, "CustomerID").ToString() + "\" onClick=\"DeleteCustomerLevel(" + DB.RowFieldInt(row, "CustomerID").ToString() + ")\" class=\"normalButtons\"></td>\n");
                    writer.Write("</tr>\n");
                }
                    ...
                    writer.Write("<input type=\"submit\" value=\"Add Customer To This Level\" name=\"Submit\" class=\"normalButtons\">");
                    writer.Write("</p>\n");
                    writer.Write("</form>\n");
