    ...
    public partial class entityBulkShipping : System.Web.UI.Page
    {
        ...
        protected void LoadBody()
        {
            ...
            using (ProductCollection products = new ProductCollection(m_EntitySpecs.m_EntityName, EntityID))
            {
                ...
                using (DataSet dsProducts = products.LoadFromDB())
                {
                    ...
                    if (NumProducts > 0)
                    {
                        using (DataTable dtShippingMethods = new DataTable())
                        {
                            ...
                            tmpS.Append("    <tr >\n");
                            tmpS.Append("      <td colspan=\"8\" align=\"right\"><input type=\"submit\" value=\"Shipping Update\" name=\"Submit\" class=\"normalButtons\"></td>\n");
                            tmpS.Append("    </tr>\n");
                            ...
                            for (int i = 0; i < dsProducts.Tables[0].Rows.Count; i++)
                            {
                                ...
                                tmpS.Append("</td>");
                                tmpS.Append("<td align=\"left\" valign=\"top\">");
                                tmpS.Append("<a target=\"entityBody\" href=\"entityeditproducts.aspx?iden=" + ThisProductID.ToString() + "&entityname=" + EntityName + "&entityid=" + EntityID.ToString() + "\">");
                                tmpS.Append(DB.RowFieldByLocale(row, "Name", cust.LocaleSetting));
                                tmpS.Append("</a>");
                                tmpS.Append("</td>\n");
                                tmpS.Append("<td align=\"left\" valign=\"top\">");
                                tmpS.Append("<a target=\"entityBody\" href=\"entityeditproductvariant.aspx?productid=" + ThisProductID.ToString() + "&variantid=" + ThisVariantID.ToString() + "&entityname=" + EntityName + "&entityid=" + EntityID.ToString() + "\">");
                            ...
                            tmpS.Append("    <tr>\n");
                            tmpS.Append("      <td colspan=\"8\" align=\"right\"><input type=\"submit\" class=\"normalButtons\" value=\"Shipping Update\" name=\"Submit\"></td>\n");
                            tmpS.Append("    </tr>\n");
                            tmpS.Append("</table>\n");
