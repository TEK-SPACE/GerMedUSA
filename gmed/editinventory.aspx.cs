            ...
            using (SqlConnection dbconn = DB.dbConn())
            {
                ...
                using (IDataReader rs = DB.GetRS("select * from productvariant   with (NOLOCK)  where VariantID=" + VariantID.ToString(), dbconn))
                {
                    ...
                    if (ErrorMsg.Length == 0)
                    {
                        ...
                        writer.Write(AppLogic.GetInventoryTable(ProductID, VariantID, true, SkinID, false, true));

                        writer.Write("<p align=\"left\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input Class=\"normalButtons\" type=\"submit\" value=\"Update\" name=\"submit\">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<input Class=\"normalButtons\" type=\"reset\" value=\"Reset\" name=\"reset\"></p>\n");
                        writer.Write("</form>\n");
                        writer.Write("</div>");
                    }
                }
