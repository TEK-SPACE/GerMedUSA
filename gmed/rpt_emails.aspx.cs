...
namespace AspDotNetStorefrontAdmin
{
    ...
    public partial class rpt_EMails : AspDotNetStorefront.SkinBase
    {
        ...
        protected override void RenderContents(System.Web.UI.HtmlTextWriter writer)
        {
            ...
            string SuperuserFilter = CommonLogic.IIF(ThisCustomer.IsAdminSuperUser, String.Empty, " Customer.CustomerID not in (select CustomerID from Customer where IsAdmin<>3) and ");

            DateTime MinCustomerDate = Localization.ParseUSDateTime("1/1/1990");
            using (SqlConnection dbconn = DB.dbConn())
            {
                dbconn.Open();
                using (IDataReader rsd = DB.GetRS("Select min(CreatedOn) as MinDate from customer  with (NOLOCK)  ", dbconn))
                {
                    if (rsd.Read())
                    {
            ...
            writer.Write("    </tr>");
            writer.Write("    <tr>");
            writer.Write("      <td style=\"border-top:solid 2px #1B427D; background-color:#ffffff\" width=\"100%\" valign=\"middle\" align=\"center\" bgcolor=\"#dfecff\" height=\"25px\" colspan=\"2\">");
            writer.Write("        <input type=\"submit\" class=\"normalButtons\" value=\"Submit\" name=\"B1\">&nbsp;<input class=\"normalButtons\" type=\"button\" onClick=\"javascript:self.location='rpt_EMails.aspx';\" value=\"Reset\" name=\"B2\">");
            writer.Write("      </td>");
            writer.Write("    </tr>");
            writer.Write("  </table>");
            writer.Write("</form>");
