...
namespace AspDotNetStorefrontAdmin
{
    ...
    public partial class cst_editaddress : AspDotNetStorefront.SkinBase
    {
        ...
        protected override void RenderContents(System.Web.UI.HtmlTextWriter writer)
        {
            ...
            bool CanDelete = (0 == DB.GetSqlN(String.Format("select count(0) as N from ShoppingCart   with (NOLOCK)  where (ShippingAddressID={0} or BillingAddressID={0}) and CartType={1}", AddressID, (int)CartTypeEnum.RecurringCart)));

            writer.Write("</td></tr>\n");
            writer.Write("</table>\n");
            //Button to submit the form
            writer.Write("<p align=\"center\"><input type=\"submit\" value=\"Save Edited Address\" name=\"Continue\" class=\"normalButtons\"></p>\n");
            if (CanDelete)
            {
                writer.Write("&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"submit\" value=\"Delete This Address\" name=\"Delete\" onClick=\"if (confirm('Do you want to delete this address permanently?')) {EditAddressForm.DeleteAddressID.value=" + AddressID.ToString() + ";} \" class=\"normalButtons\"></p>");
            }
            else
            {
                writer.Write("&nbsp;&nbsp;&nbsp;&nbsp;<input type=\"button\" value=\"Delete This Address\" name=\"Delete\" onClick=\"alert('You may not delete this address because it is being used by an Auto-Ship order. You will need to remove it from all Auto-Ship orders before it may be deleted.')  \" class=\"normalButtons\"></p>");
            }
            writer.Write("</form>\n");
            writer.Write("</td></tr>\n");
            writer.Write("</table>\n");
