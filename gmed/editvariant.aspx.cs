        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (CommonLogic.FormBool("IsSubmit"))
            {
                ...
                if (!Editing)
                {
                    ...
                    sql.Append(DB.SQuote(CommonLogic.FormCanBeDangerousContent("SEAltText")) + ",");
                ...
                else
                {
                    ...
                    sql.Append("SEAltText=" + DB.SQuote(CommonLogic.FormCanBeDangerousContent("SeAlt")) + ",");
                    if (AppLogic.FormLocaleXml("Description").Length != 0)
                    {
                ...
                if (ShipCalcID == Shipping.ShippingCalculationEnum.UseIndividualItemShippingCosts)
                {
                    ...
                    {
                        dbconn.Open();
                        using (IDataReader rs3 = DB.GetRS("select * from ShippingMethod  with (NOLOCK)  where IsRTShipping=0 order by DisplayOrder", dbconn))
                        {
                            while (rs3.Read())
                            {
        ...
        protected override void RenderContents(System.Web.UI.HtmlTextWriter writer)
        {
            ...
            using (SqlConnection dbconn = DB.dbConn())
            {
                ...
                using (IDataReader rs = DB.GetRS("select PV.*,P.ColorOptionPrompt,P.SizeOptionPrompt from Product P  with (NOLOCK)  , productvariant PV   with (NOLOCK)  where PV.ProductID=P.ProductID and VariantID=" + VariantID.ToString(), dbconn))
                {
                    ...
                    String ColorOptionPrompt = DB.RSFieldByLocale(rs, "ColorOptionPrompt", Localization.GetWebConfigLocale());
                    String SizeOptionPrompt = DB.RSFieldByLocale(rs, "SizeOptionPrompt", Localization.GetWebConfigLocale());
                    ...
                    if (ErrorMsg.Length == 0)
                    {
                        ...
                        if (Editing)
                        {
                            ...
                            int NumVariants = DB.GetSqlN(String.Format("select count(*) as N from ProductVariant with (NOLOCK) where Deleted=0 and ProductID={0}", ProductID.ToString()));
                            writer.Write("&nbsp;&nbsp;&nbsp;&nbsp;");
                            if (NumVariants > 1)
                            {
                        ...
                        if (ShipCalcID == Shipping.ShippingCalculationEnum.UseIndividualItemShippingCosts)
                        {
                            ...
                            {
                                dbconn2.Open();
                                using (IDataReader rs3 = DB.GetRS("select * from ShippingMethod  with (NOLOCK)  where IsRTShipping=0 order by DisplayOrder", dbconn2))
                                {
                                    while (rs3.Read())
                                    {
                        ...
                        writer.Write("                <td align=\"left\" valign=\"top\">\n");
                        writer.Write("Yes&nbsp;<INPUT TYPE=\"RADIO\" NAME=\"FreeShipping\" value=\"1\" " + CommonLogic.IIF(Editing, CommonLogic.IIF(DB.RSFieldTinyInt(rs, "FreeShipping") == 1, " checked ", ""), "  ") + ">\n");
                        writer.Write("No&nbsp;<INPUT TYPE=\"RADIO\" NAME=\"FreeShipping\" value=\"0\" " + CommonLogic.IIF(Editing, CommonLogic.IIF(DB.RSFieldTinyInt(rs, "FreeShipping") == 0, " checked ", ""), " checked ") + ">\n");
                        writer.Write("No Shipping Required&nbsp;<INPUT TYPE=\"RADIO\" NAME=\"FreeShipping\" value=\"2\" " + CommonLogic.IIF(Editing, CommonLogic.IIF(DB.RSFieldTinyInt(rs, "FreeShipping") == 2, " checked ", ""), " ") + ">\n");
                        writer.Write("                </td>\n");
                        writer.Write("              </tr>\n");
                        ...
                        else
                        {
                            ...
                            writer.Write("                <td align=\"left\" valign=\"top\">\n");
                            writer.Write("                	<input maxLength=\"9\" size=\"10\" name=\"Inventory\" value=\"" + CommonLogic.IIF(Editing, DB.RSFieldInt(rs, "Inventory").ToString(), "") + "\">\n");
                        ...
                        if (!AppLogic.ProductIsMLExpress() && !AppLogic.ProductIsMLX())
                        {
                            ...
                            {
                                writer.Write("              <tr valign=\"middle\">\n");
                                writer.Write("                <td align=\"right\" valign=\"middle\">*Is Subscription:&nbsp;&nbsp;</td>\n");
                                writer.Write("                <td align=\"left\" >\n");
                                writer.Write("Yes&nbsp;<INPUT TYPE=\"RADIO\" NAME=\"SubscriptionInterval\" value=\"1\" " + CommonLogic.IIF(Editing, CommonLogic.IIF(DB.RSFieldBool(rs, "SubscriptionInterval"), " checked ", ""), "") + ">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;\n");
                                writer.Write("No&nbsp;<INPUT TYPE=\"RADIO\" NAME=\"SubscriptionInterval\" value=\"0\" " + CommonLogic.IIF(Editing, CommonLogic.IIF(DB.RSFieldBool(rs, "SubscriptionInterval"), "", " checked "), " checked ") + ">\n");
                                writer.Write("                </td>\n");
                                writer.Write("              </tr>\n");
                                writer.Write("              <tr valign=\"middle\">\n");
                                writer.Write("                <td align=\"right\" valign=\"middle\">*Subscription Interval Type:&nbsp;&nbsp;</td>\n");
                                ...
                                writer.Write("<select size=\"1\" name=\"SubscriptionIntervalType\">\n");

                                DateIntervalTypeEnum e;
                                e = DateIntervalTypeEnum.Weekly;
                                writer.Write("<option value=\"" + ((int)e).ToString() + "\"");
                                if (Editing && DB.RSFieldInt(rs, "SubscriptionIntervalType") == (int)e)
                                {
                                    writer.Write(" selected");
                                }
                                writer.Write(">" + e.ToString() + "</option>");
                        ...
                        writer.Write("                <td align=\"left\" valign=\"top\">\n");
                        writer.Write("                	<input maxLength=\"100\" size=\"40\" name=\"SeAlt\" value=\"" + CommonLogic.IIF(Editing, DB.RSField(rs, "SeAltText"), "") + "\">\n");
                        writer.Write("                </td>\n");
                        writer.Write("              </tr>\n");
                        ...
                        if (Editing && (DB.RSField(rs, "Colors").Length != 0 || DB.RSField(rs, "ColorSKUModifiers").Length != 0 || DB.RSField(rs, "Sizes").Length != 0 || DB.RSField(rs, "SizeSKUModifiers").Length != 0))
                        {
                            ...
                            String[] Colors = DB.RSFieldByLocale(rs, "Colors", Localization.GetWebConfigLocale()).Split(',');
                            String[] Sizes = DB.RSFieldByLocale(rs, "Sizes", Localization.GetWebConfigLocale()).Split(',');
                            String[] ColorSKUModifiers = DB.RSFieldByLocale(rs, "ColorSKUModifiers", Localization.GetWebConfigLocale()).Split(',');
                            String[] SizeSKUModifiers = DB.RSFieldByLocale(rs, "SizeSKUModifiers", Localization.GetWebConfigLocale()).Split(',');
