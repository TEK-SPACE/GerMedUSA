...
namespace AspDotNetStorefront
{
    ...
    public partial class ajaxShipping : System.Web.UI.Page
    {
                        ...
                        using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
                        {
                            ...
                            using (IDataReader rs = DB.GetRS(shipsql.ToString(), con))
                            {
                                while (rs.Read())
                                {
                                    ...
                                    {
                                        writer.WriteElementString("Method" + ThisID.ToString(), Shipping.GetShippingMethodName(ThisID, null) + ": " + AppLogic.GetString("shoppingcart.aspx.16", ThisCustomer.SkinID, ThisCustomer.LocaleSetting));
                                    }
                                    else
                                    {
                ...
                case Shipping.ShippingCalculationEnum.UseRealTimeRates:
                    {
                        ...
                        RTShipping.ShippingMethods SM = GetRates();

                        if (SM.Count > 0 )
                        {
                            int ThisID = 1;
                            foreach (RTShipping.ShippingMethod meth in SM)
                            {
        ...
        protected RTShipping.ShippingMethods GetRates()
        {
            ...
            RTShipping.Shipments shipments = new RTShipping.Shipments();
            ...
            shipments = new RTShipping.Shipments();
            ...
            return (RTShipping.ShippingMethods)realTimeShipping.GetRates(shipments, carriers, format, "ShippingMethod", "ShippingMethod", ShippingTaxRate, out RTShipRequest, out RTShipResponse, ShippingHandlingExtraFee, (decimal)MarkupPercent, realTimeShipping.ShipmentValue);
        }
    }
}
