...
namespace AspDotNetStorefront
{
	...
	public partial class ratecomment : System.Web.UI.Page
	{
		protected void Page_Load(object sender, System.EventArgs e)
		{
            ...
            {
                conn.Open();
                using (IDataReader rs = DB.GetRS("select * from RatingCommentHelpfulness   with (NOLOCK)  where ProductID=" + ProductID.ToString() + " and RatingCustomerID=" + CustomerID.ToString() + " and VotingCustomerID=" + VotingCustomerID.ToString(), conn))
                {
                    if (rs.Read())
                    {
                        AlreadyVoted = true;
