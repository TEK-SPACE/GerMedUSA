            ...
            if (CustomerID > 0)
            {
                ...
                if (rawResponse.Equals("VERIFIED", StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    if (ExistingOrderNumber == 0)
                    {
                        ...
                        {
                            ExistingOrderNumber = DB.GetSqlN("select min(ordernumber) N from orders where paymentmethod = '" + AppLogic.ro_PMPayPal + "' AND charindex(" + DB.SQuote(parent_txn_id) + ",AuthorizationPNREF) > 0");
                        }
                        else
                        {
                            ExistingOrderNumber = DB.GetSqlN("select min(ordernumber) N from orders where paymentmethod = '" + AppLogic.ro_PMPayPal + "' AND charindex(" + DB.SQuote(txn_id) + ",AuthorizationPNREF) > 0");
                        }
                    }
