        ...
        protected void Page_Load(object sender, System.EventArgs e)
        {
            ...
            if (!ThisCustomer.IsRegistered && AppLogic.AppConfigBool("DisallowAnonCustomerToCreateWishlist") && AppLogic.ProductIsMLExpress() == false)
            {
                ...
                if (CommonLogic.FormNativeInt("IsWishList") == 1 || CommonLogic.QueryStringUSInt("IsWishList") == 1)
                {
                    ...
                    Response.Redirect("signin.aspx?ErrorMsg=" + ErrMsg + "&ReturnUrl=" + Security.UrlEncode(ReturnURL));
                }
            ...
            if (ShoppingCartRecID != 0)
            {
                DB.ExecuteSQL("delete from ShoppingCart where ShoppingCartRecID=" + ShoppingCartRecID.ToString() + " and CustomerID=" + ThisCustomer.CustomerID.ToString() + " and CartType=" + ((int)CartType).ToString());
            }
            ...
            if (Quantity > 0)
            {
                ...
                else if (AppLogic.IsAPack(ProductID) && CommonLogic.QueryStringCanBeDangerousContent("UpdateCartPack") == "")
                {
                    ...
                    {
                        con.Open();
                        using (IDataReader dr = DB.GetRS("select RequiresTextOption from dbo.Product where ProductID=" + ProductID.ToString(), con))
                        {
                            dr.Read();
                            RequiresTextOption = DB.RSFieldBool(dr, "RequiresTextOption");
                        }
                    }
                    ...
                    if (NewRecID == 0)
                    {
                        NewRecID = cart.AddItem(ThisCustomer, ShippingAddressID, ProductID, VariantID, Quantity, ChosenColor, ChosenColorSKUModifier, ChosenSize, ChosenSizeSKUModifier, TextOption, CartType, false, false, 0, System.Decimal.Zero);
                    }
                }
                else
                {
                                ...
                                if (UpsellVariantID != 0)
                                {
                                    ...
                                    using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
                                    {
                                        ...
                                        using (IDataReader rs = DB.GetRS("select Sizes,SizeSKUModifiers,Colors,ColorSKUModifiers from ProductVariant  with (NOLOCK)  where VariantID=" + UpsellVariantID.ToString(), con))
                                        {
                                            if (rs.Read())
                                            {
                                                Sizes = DB.RSFieldByLocale(rs, "Sizes", Localization.GetWebConfigLocale());
                                                SizeSKUModifiers = DB.RSFieldByLocale(rs, "SizeSKUModifiers", Localization.GetWebConfigLocale());
                                                Colors = DB.RSFieldByLocale(rs, "Colors", Localization.GetWebConfigLocale());
                                                ColorSKUModifiers = DB.RSFieldByLocale(rs, "ColorSKUModifiers", Localization.GetWebConfigLocale());
                                            }
                                        }
                                    }
