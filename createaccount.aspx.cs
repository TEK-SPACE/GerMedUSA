    ...
    public partial class createaccount : SkinBase
    {
        ...
        public void BillingCountry_OnChange(object sender, EventArgs e)
        {
            ...
            if (BillingState.Items.Count == 0)
            {
                ...
                {
                    con.Open();
                    using (IDataReader dr = DB.GetRS("select * from State  with (NOLOCK)  where countryid is null", con))
                    {
                        if (dr.Read())
                        {
                            BillingState.Items.Insert(0, new ListItem(DB.RSField(dr, "Name"), DB.RSField(dr, "Abbreviation")));
                        }
                        else
                        {
        ...
        protected void valRequireOver13_ServerValidate(object source, ServerValidateEventArgs args)
        {
            if (AppLogic.AppConfigBool("RequireOver13Checked"))
            {
                ...
                {
                    args.IsValid = false;
                    valRequireOver13.ErrorMessage = AppLogic.GetString("Over13OnCreateAccount", SkinID, ThisCustomer.LocaleSetting);
                }
                else
                {
                    args.IsValid = true;
        ...
        private void InitializePageContent()
        {
            ...
            PasswordPanel.Visible = (ThisCustomer.Password == "" || ThisCustomer.Password == p.SaltedPassword);
            ...
            if (!SkipRegistration)
            {
                ...
                if (Checkout && ((AppLogic.AppConfigBool("PasswordIsOptionalDuringCheckout") && AppLogic.ProductIsMLExpress() == false) || AppLogic.AppConfigBool("HidePasswordFieldDuringCheckout")))
                {
            ...
            if (!IsPostBack)
            {
                ...
                if (AllowShipToDifferentThanBillTo)
                {
                    ...
                    ShippingFirstName.Text = Server.HtmlEncode(CommonLogic.IIF(ThisCustomer.FirstName.Length != 0, ThisCustomer.FirstName, BillingAddress.FirstName));
        ...
        private void CreateAccount()
        {
            ...
            if (Page.IsValid && AccountName.Length > 0)
            {
                ...
                if (!EMailAlreadyTaken)
                {
                    ...
                    if (vtr.Length > 0 && (!AppLogic.AppConfigBool("VAT.Enabled") || !AppLogic.VATRegistrationIDIsValid(BillingCountry.SelectedValue, vtr)))
                    {
                        vtr = String.Empty;
                    }
            ...
            {
                ErrorMsgLabel.Text += "<br /><br />" + AppLogic.GetString("createaccount.aspx.84", 1, Localization.GetWebConfigLocale()) + "<br /><br />";
            ...
            if (ShippingState.Items.Count == 0)
            {
                using (SqlConnection con = new SqlConnection(DB.GetDBConn()))
                {
                    ...
                    using (IDataReader dr = DB.GetRS("select * from State  with (NOLOCK)  where countryid is null", con))
                    {
                        if (dr.Read())
                        {
                            ShippingState.Items.Insert(0, new ListItem(DB.RSField(dr, "Name"), DB.RSField(dr, "Abbreviation")));
                        }
                        else
                        {
        ...
        private void SetPasswordFields()
        {
            ...
            if (txtpassword.Text.Trim() != "" && Regex.IsMatch(txtpassword.Text.Trim(), "[^\xFF]", RegexOptions.Compiled))
            {
            ...
            if (txtpassword2.Text != "" && Regex.IsMatch(txtpassword2.Text.Trim(), "[^\xFF]", RegexOptions.Compiled))
            {
